import Link from "next/link";

export const NeonetDescription = () => {
  return (
    <>
      <p>
        As a Junior Frontend Developer, I was mainly tasked with creating
        features, fixing, and maintaining frontend functionality of our internal
        application that was a dashboard which allowed to visualize data for
        departaments like Marketing or HR.
      </p>
      <p>
        To my tasks were also included creating and modyfing Landing Pages on
        <Link href="https://www.neonet.pl"> Neonet </Link> and{" "}
        <Link href="https://www.neo24.pl"> Neo24 </Link> websites.
      </p>
      <p>
        As well as development, bug fixing, and optimization of our internal
        Magento plugin for Chrome
      </p>
    </>
  );
};
